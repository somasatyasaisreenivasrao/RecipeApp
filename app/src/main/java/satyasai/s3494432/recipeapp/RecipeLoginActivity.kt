                ...
                onClick = {
                    when {
                        ...
                        password.isBlank() -> {
                            ...
                            loginChefAccount(userData.email, userPassword = userData.password,context)
                        }
                    }
                },
                modifier = Modifier
                ...
                Text(
                    ...
                    modifier = Modifier.clickable {
                        context.startActivity(
                            Intent(
                                context,
                                RecipeRegisterActivity::class.java
                            )
                        )
                        context.finish()
                    }
...
fun loginChefAccount(userEmail: String,userPassword: String,context: Context) {
    val database = FirebaseDatabase.getInstance()
    val databaseReference = database.reference

    val sanitizedEmail = userEmail.replace(".", ",")

    databaseReference.child("ChefAccounts").child(sanitizedEmail).get()
        .addOnSuccessListener { snapshot ->
            if (snapshot.exists()) {
                val chefData = snapshot.getValue(ChefData::class.java)
                chefData?.let {
